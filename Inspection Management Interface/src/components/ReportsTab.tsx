...
export function ReportsTab() {
  const [filterTimeWindow, setFilterTimeWindow] = useState("30days");
  const [filterPart, setFilterPart] = useState("all");
  const [filterLine, setFilterLine] = useState("all");
  const [filterPhase, setFilterPhase] = useState("all");

  const handleApplyFilters = () => {
    toast.success("Filters applied - charts updated");
  };

  const handleGenerateReport = () => {
    toast.success("Report generation dialog opened");
  };
  ...
  const handleViewReport = (reportId: string) => {
    toast.info(`Opening report ${reportId} in viewer...`);
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          <Select defaultValue="30days" value={filterTimeWindow} onValueChange={setFilterTimeWindow}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
                ...
                <TableRow key={report.id}>
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => handleViewReport(report.id)}>View</Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
